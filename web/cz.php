...
class ipLocation
{
	...
	//*
	//构造函数,初始化一些变量
	//$datfile 的值为纯真IP数据库的名字,可自行修改.
	//*
	function __construct($datfile="qqwry.dat")
	{
	...
	function getIP()
	{
		...
		{
			$ip=getenv('HTTP_X_FORWARDED_FOR');  //获取客户端用代理服务器访问时的真实ip地址
		}
	...
	function getaddress($ip)
	{
		...
		//*过滤一些无用数据
		if(strstr($address["area1"],"CZ88.NET"))
		{
			$address["area1"]="未知地址";
		}
		if(strstr($address["area2"],"CZ88.NET"))
		{
			$address["area2"]="";
		}
		return $address["area1"].$address["area2"];
	}
}
